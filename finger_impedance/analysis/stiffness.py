...
with open("data_s1_1_12.pkl", "rb") as handle:
    data1 = pickle.load(handle)
with open("data_s2_1_12.pkl", "rb") as handle:
    data2 = pickle.load(handle)
with open("data_s3_1_12.pkl", "rb") as handle:
    data3 = pickle.load(handle)
with open("data_s4_1_12.pkl", "rb") as handle:
...
for i in range(0, 4):
    ...
    for k in normalized_stiffness[i, :, :].T:
        moav_stiffness2[:, index] = np.array(moving_average(k, 100))
        index = index + 1
...
print(movement_id.shape)
print(mean_min_std.shape)
...
for i in np.unique(movement_id):
    ...
    ax2[0].annotate(label_list[int(i)], xy=(int(ranges[i][0][0]), 100), fontsize=10)
...
figname = "fig/avg_" + f"{force_labels[finger_1]}" + "_" + "1.pdf"
