...
def image_features(data: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, Tuple[float, float], np.ndarray]:
...
def map_values(i: int, epoch: int) -> Tuple[float, float, float, np.ndarray, np.ndarray, float, str]:
    """Get activation map values and metadata for a given epoch index.

    Returns:
        Tuple of (co_contraction_intensity, ext_intensity, flex_intensity,
                  flex_map, ext_map, time_seconds, action_label).
    """
...
def local_maximum_pos(data: np.ndarray, threshold: float, neighborhood_size: int) -> Tuple[List[float], List[float]]:
...
# --- Dataset creation (commented out, kept for reference) ---
