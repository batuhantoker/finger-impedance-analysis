...
class tfest:
    ...
    def transfer_function_H(self,Fs, method="h1", time=None):
        """
        ...
        if method == "fft":
            ...
            frequency = fftfreq(u_f.size, d=Fs/len(self.u))[np.nonzero(u_f)]
            H = y_no_zero / u_no_zero
        elif method == "h1":
            # https://dsp.stackexchange.com/questions/71811/understanding-the-h1-and-h2-estimators
            cross_sd, frequency = csd(self.u, self.y, Fs=Fs, NFFT=len(self.u))
            power_sd, _ = psd(self.u, Fs=Fs, NFFT=len(self.u))
            H = cross_sd / power_sd
        elif method == "h2":
            cross_sd, frequency = csd(self.y, self.u, Fs=Fs, NFFT=len(self.u))
            power_sd, _ = psd(self.y, Fs=Fs, NFFT=len(self.u))
            H = power_sd / cross_sd
        else:
    ...
    def estimate(self, nzeros, npoles,Fs, init_value=1, options={'xatol': 1e-3, 'disp': True}, method="h1", time=None,
                 l1=0):
        """
