"""Main pipeline for Malesevic dataset.

Loads per-subject .mat files (subjects 1–20, skipping 5), applies EMG filtering
and feature extraction, estimates finger stiffness, and saves results to per-subject
pickle files in males/.
"""
...
for i in subject_range:
    ...
    movement_ranges_0 = movement_ranges_0all[0][
        0 : (round(movement_ranges.shape[0] / (movement_f - movement) + 1))
    ]
    ...
    valid_classes = np.r_[
        np.array([i for i, v in enumerate(emg_class) if v.is_integer()])
    ]

    RMS, MAV, IAV, VAR, WL, MF, PF, MP, TP, SM = feature_extraction(emg_ext_pp, epoch)
    dict_ext = {
        "rms_ext": RMS[valid_classes, :],
        "mav_ext": MAV[valid_classes, :],
        "iav_ext": IAV[valid_classes, :],
        "var_ext": VAR[valid_classes, :],
        "wl_ext": WL[valid_classes, :],
        "mf_ext": MF[valid_classes, :],
        "pf_ext": PF[valid_classes, :],
        "mp_ext": MP[valid_classes, :],
        "tp_ext": TP[valid_classes, :],
        "sm_ext": SM[valid_classes, :],
    }
    RMS, MAV, IAV, VAR, WL, MF, PF, MP, TP, SM = feature_extraction(emg_flex_pp, epoch)
    dict_flex = {
        "rms_flex": RMS[valid_classes, :],
        "mav_flex": MAV[valid_classes, :],
        "iav_flex": IAV[valid_classes, :],
        "var_flex": VAR[valid_classes, :],
        "wl_flex": WL[valid_classes, :],
        "mf_flex": MF[valid_classes, :],
        "pf_flex": PF[valid_classes, :],
        "mp_flex": MP[valid_classes, :],
        "tp_flex": TP[valid_classes, :],
        "sm_flex": SM[valid_classes, :],
    }
