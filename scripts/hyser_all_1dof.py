"""Summarize dimensionless activation proxies from Hyser schema-v2 NPZ files."""
...
DEFAULT_INPUT_GLOB = "hyser_processed/subject*_session*.npz"
DEFAULT_OUTPUT = Path("hyser_activation_proxy_summary.csv")


def build_parser() -> argparse.ArgumentParser:
    """Build the command-line parser."""
    parser = argparse.ArgumentParser(
        description="Write a CSV summary of Hyser dimensionless activation proxies.",
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the Hyser activation-proxy summary CLI."""
    parser = build_parser()
    args = parser.parse_args(argv)
    paths = _input_paths(args.files)
    if not paths:
        parser.error(
            f"no Hyser schema-v2 NPZ files matched {DEFAULT_INPUT_GLOB!r}; "
